...
import Select2ChoiceFilter from 'oro/datafilter/select2-choice-filter';
import initSelect2 from 'pim/initselect2';
import ChoiceFilterCriteria from './ChoiceFilterCriteria';

/**
 * React inner-render of the select2-choice datagrid filter (C1 Wave 5).
 *
 * Extends the legacy `select2-choice-filter` (which extends TextFilter) to inherit ALL its Select2
 * behaviour — the ajax/static `_getSelect2Config`, the resultCache label hydration, the value path
 * (`_getInputValue`/`_setInputValue` = `select2('val'/'data')`), the open/close flow — and overrides
 * only the markup + operator methods, swapping the underscore template for the shared React
 * `ChoiceFilterCriteria` (whose memoized `React.memo(()=>true)` ValueField keeps the Select2-decorated
 * `input[name="value"]` stable across re-renders).
 *
 ...
 *    and `_getCriteriaHint` read it directly, not the `.active .operator_choice` DOM (avoids the
 *    one-render lag of reading the DOM that `_renderReact` is about to replace).
 *  - jQuery keeps owning popup VISIBILITY (`_showCriteria`/`_hideCriteria`), the value path + Select2
 *    (the memoized ValueField), and the empty/not-empty enable/disable of the input.
 *
 * Unlike choice-filter-react there is no `_toggleListSelection` shell to init Select2, so `render()`
 * inits it itself (as the legacy `_renderCriteria` did), and `remove()` destroys it (the legacy never
 * did — Select2 portals its dropdown to `<body>`, so unmounting without destroy orphans it: the D5 leak).
 *
 * Added ALONGSIDE `select2-choice-filter.js`; only the `select2-choice` FilterTypeRegistry alias is
 * re-pointed. `select2-rest-choice` (a sibling, not a subclass) stays legacy for now.
 */
export default Select2ChoiceFilter.extend({
  /**
   * {@inheritdoc}
   *
