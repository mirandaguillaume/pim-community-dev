// Unit test for the `select2-choice-filter-react` bridge: `export default Select2ChoiceFilter.extend({})`.
// Mock each legacy AMD dep BEFORE importing the bridge (jest.mock is hoisted); the base mock uses the
// `backboneExtend` technique so the bridge's `Base.prototype.method.call(this)` super-calls resolve.
...
// The bridge super-calls TextFilter.prototype._showCriteria/_hideCriteria/_onValueUpdated.
jest.mock(
...
// The BASE the bridge extends. Constructor seeds the instance fields the bridge reads; the prototype
// carries the inherited methods the bridge calls (_getSelect2Config, _getDisplayValue, _getInputValue,
// _disableInput, _enableInput, getValue); it exposes the Backbone `.extend`.
jest.mock(
  'oro/datafilter/select2-choice-filter',
  () => {
    function Select2ChoiceFilter(this: any) {
    ...
    const proto = (Select2ChoiceFilter as any).prototype;
    ...
    (Select2ChoiceFilter as any).extend = backboneExtend;

    return Select2ChoiceFilter;
  },
  {virtual: true}
);
...
// Stub the popup component: reflect the forwarded props into data-attributes so `_renderReact` mutants die.
...
import Bridge from '../../../Resources/public/js/datafilter/filter/select2-choice-filter-react';

const stub = (filter: any): HTMLElement | null => filter.el.querySelector('.criteria-stub');
const s2Proto = (): any => jest.requireMock('oro/datafilter/select2-choice-filter').prototype;
...
describe('remove()', () => {
  test('destroys Select2, unmounts the React tree, chains to AbstractFilter.remove', () => {
    ...
    expect(stub(filter)).toBeNull(); // unmounted
  ...
  test('does not destroy Select2 when it was never initialised', () => {
    const filter: any = new Bridge();
    filter._$el.data = jest.fn(() => undefined); // no select2 instance
