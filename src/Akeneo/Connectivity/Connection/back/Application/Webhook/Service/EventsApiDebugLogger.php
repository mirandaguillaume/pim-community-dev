    ...
    public function logEventsApiRequestSucceed(
        ...
        $this->repository->persist([
            ...
            'context' => [
                ...
                'headers' => $headers,
                'events' => \array_map(fn (\Akeneo\Connectivity\Connection\Domain\Webhook\Model\WebhookEvent $webhookEvent): array => $this->normalizeEvent($webhookEvent->getPimEvent()), $events),
            ],
        ]);
    }
    ...
    public function logEventsApiRequestFailed(string $connectionCode, array $webhookEvents, string $url, int $statusCode, array $headers): void
    {
        $this->repository->persist([
            ...
            'context' => [
                ...
                'status_code' => $statusCode,
                'headers' => $headers,
                'events' => \array_map(fn (\Akeneo\Connectivity\Connection\Domain\Webhook\Model\WebhookEvent $webhookEvent): array => $this->normalizeEvent($webhookEvent->getPimEvent()), $webhookEvents),
            ],
        ]);
    }
