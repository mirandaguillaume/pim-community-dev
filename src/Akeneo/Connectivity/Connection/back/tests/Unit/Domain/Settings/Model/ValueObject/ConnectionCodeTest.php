...
class ConnectionCodeTest extends TestCase
{
    ...
    {
        $this->expectException(new \InvalidArgumentException('akeneo_connectivity.connection.connection.constraint.code.required'));
        new ConnectionCode('');
    }

    public function test_it_cannot_contains_a_string_shorter_than_3_characters(): void
    {
        $this->expectException(new \InvalidArgumentException('akeneo_connectivity.connection.connection.constraint.code.too_short'));
        new ConnectionCode('aa');
    }

    public function test_it_cannot_contains_a_string_longer_than_100_characters(): void
    {
        $this->expectException(new \InvalidArgumentException('akeneo_connectivity.connection.connection.constraint.code.too_long'));
        new ConnectionCode(\str_repeat('a', 103));
    }

    public function test_it_contains_only_alphanumeric_characters(): void
    {
        $this->expectException(new \InvalidArgumentException('akeneo_connectivity.connection.connection.constraint.code.invalid'));
