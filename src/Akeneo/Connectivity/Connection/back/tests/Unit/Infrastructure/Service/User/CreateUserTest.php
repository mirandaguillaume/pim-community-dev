...
class CreateUserTest extends TestCase
{
    ...
    public function test_it_creates_a_user(): void
    {
        ...
        $this->userUpdater->expects($this->once())->method('update')->with($user, $this->isType('array'));
        ...
        $readUser = $this->sut->execute('foo', 'bar', 'baz');
        $this->assertInstanceOf(ReadUser::class, $readUser);
        $this->assertSame(1, $readUser->id());
        $this->assertIsString($readUser->username());
        $this->assertIsString($readUser->password());
    }
