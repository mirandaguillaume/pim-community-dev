        ...
        foreach ($data as $associationType => $associations) {
            if (\array_key_exists('product_uuids', $associations)) {
                $productQuantityValues = \array_map(
                    fn (array $association): \Akeneo\Pim\Enrichment\Product\API\Command\UserIntent\QuantifiedAssociation\QuantifiedEntity => new QuantifiedEntity($association['uuid'], $association['quantity']),
            ...
            if (\array_key_exists('products', $associations)) {
                $productQuantityValues = \array_map(
                    fn (array $association): \Akeneo\Pim\Enrichment\Product\API\Command\UserIntent\QuantifiedAssociation\QuantifiedEntity => new QuantifiedEntity($association['identifier'], $association['quantity']),
