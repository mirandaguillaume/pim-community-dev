...
class SetFamilyApplierTest extends TestCase
{
    private ObjectUpdaterInterface|MockObject $updater;
    private SetFamilyApplier $sut;

    protected function setUp(): void
    {
        $this->updater = $this->createMock(ObjectUpdaterInterface::class);
        $this->sut = new SetFamilyApplier($this->updater);
    }

    public function test_it_is_initializable(): void
    {
        $this->assertInstanceOf(SetFamilyApplier::class, $this->sut);
    ...
    public function test_it_applies_set_family_user_intent(): void
    {
        ...
        $this->updater->expects($this->once())->method('update')->with($product, ['family' => 'other']);
        $this->sut->apply($setFamily, $product, 1);
    }
