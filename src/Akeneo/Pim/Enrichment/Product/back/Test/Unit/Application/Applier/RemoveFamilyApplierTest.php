...
class RemoveFamilyApplierTest extends TestCase
{
    private ObjectUpdaterInterface|MockObject $updater;
    private RemoveFamilyApplier $sut;

    protected function setUp(): void
    {
        $this->updater = $this->createMock(ObjectUpdaterInterface::class);
        $this->sut = new RemoveFamilyApplier($this->updater);
    }

    public function test_it_is_initializable(): void
    {
        $this->assertInstanceOf(RemoveFamilyApplier::class, $this->sut);
        $this->assertInstanceOf(UserIntentApplier::class, $this->sut);
    }

    public function test_it_applies_set_family_user_intent(): void
    {
        $product = new Product();
        $removeFamily = new RemoveFamily();
        $this->updater->expects($this->once())->method('update')->with($product, ['family' => null]);
        $this->sut->apply($removeFamily, $product, 1);
    }
