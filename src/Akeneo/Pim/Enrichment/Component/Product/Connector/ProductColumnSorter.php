    ...
    public function sort(array $columns, array $context = [])
    {
        if (isset($context['filters']['structure']['attributes'])
            ...
            $rawColumns = array_merge(
                ...
                array_map(fn (object $associationType) => $associationType->getCode(), $this->associationTypeRepository->findAll()),
                $context['filters']['structure']['attributes']
            );

            $sortedColumns = [];
            foreach ($rawColumns as $columnCode) {
                $sortedColumns = array_merge($sortedColumns, array_filter($columns, fn ($columnCandidate): bool => 0 !== preg_match(sprintf('/^%s(-.*)*$/', $columnCode), (string) $columnCandidate)));
            }
