...
class EvaluateCompletenessTest extends TestCase
{
    private GetLocalesByChannelQueryInterface|MockObject $localesByChannelQuery;
    private EvaluateCompleteness $sut;

    protected function setUp(): void
    {
        $this->localesByChannelQuery = $this->createMock(GetLocalesByChannelQueryInterface::class);
        $this->sut = new EvaluateCompleteness($this->localesByChannelQuery);
    }

    public function test_it_evaluates_completeness_of_a_product(): void
    {
        $completenessCalculator = $this->createMock(CalculateProductCompletenessInterface::class);
        ...
        $completenessCalculator->method('calculate')->with($productUuid)->willReturn((new Write\CompletenessCalculationResult())
        ...
        ;
        $this->assertEquals($expectedResult, $this->sut->evaluate($completenessCalculator, $criterionEvaluation));
    }
}
