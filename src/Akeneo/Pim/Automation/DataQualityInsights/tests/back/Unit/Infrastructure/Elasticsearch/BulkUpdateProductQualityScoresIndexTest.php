...
use Akeneo\Pim\Automation\DataQualityInsights\Infrastructure\Elasticsearch\BulkUpdateProductQualityScoresIndex;
...
class BulkUpdateProductQualityScoresIndexTest extends TestCase
{
    private BulkUpdateProductQualityScoresIndex $sut;

    protected function setUp(): void
    {
    }

    public function test_it_does_not_update_when_type_is_incorrect(): void
    {
        $esClient = $this->createMock(Client::class);
        $getProductScoresQuery = $this->createMock(GetProductScoresQueryInterface::class);
        $getProductModelScoresQuery = $this->createMock(GetProductModelScoresQueryInterface::class);
        $computeProductsKeyIndicators = $this->createMock(ComputeProductsKeyIndicators::class);

        $this->sut = new BulkUpdateProductQualityScoresIndex(
            $esClient,
        ...
        $this->sut->__invoke($this->getProductsData()['productUuidCollection']);
    }

    public function test_it_updates_products_index(): void
    {
        $esClient = $this->createMock(Client::class);
        $getProductScoresQuery = $this->createMock(GetProductScoresQueryInterface::class);
        $getProductModelScoresQuery = $this->createMock(GetProductModelScoresQueryInterface::class);
        ...
        $getProductScoresQuery->method('byProductUuidCollection')->with($productUuids)->willReturn($this->getProductsData()['scores']);
        $productsKeyIndicators = $this->getProductsData()['keyIndicators'];
        $computeProductsKeyIndicators->method('compute')->with($productUuids)->willReturn($productsKeyIndicators);
        $esClient->expects($this->once())->method('bulkUpdate')->with(
            [
        ...
        $this->sut->__invoke($productUuids);
    }

    public function test_it_updates_product_models_index(): void
    {
        $esClient = $this->createMock(Client::class);
        $getProductScoresQuery = $this->createMock(GetProductScoresQueryInterface::class);
        $getProductModelScoresQuery = $this->createMock(GetProductModelScoresQueryInterface::class);
        ...
        $getProductModelScoresQuery->method('byProductModelIdCollection')->with($productModelIds)->willReturn($this->getProductModelData()['scores']);
        $productModelsKeyIndicators = $this->getProductModelData()['keyIndicators'];
        $computeProductsKeyIndicators->method('compute')->with($productModelIds)->willReturn($productModelsKeyIndicators);
        $esClient->expects($this->once())->method('bulkUpdate')->with(
        ...
        $this->sut->__invoke($productModelIds);
    }
