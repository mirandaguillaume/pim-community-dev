...
class PurgeOutdatedDataTest extends TestCase
{
    private DashboardScoresProjectionRepositoryInterface|MockObject $dashboardRatesProjectionRepository;
    private ProductScoreRepositoryInterface|MockObject $productScoreRepository;
    private PurgeOutdatedData $sut;

    protected function setUp(): void
    {
        $this->dashboardRatesProjectionRepository = $this->createMock(DashboardScoresProjectionRepositoryInterface::class);
        $this->productScoreRepository = $this->createMock(ProductScoreRepositoryInterface::class);
        $this->sut = new PurgeOutdatedData($this->dashboardRatesProjectionRepository, $this->productScoreRepository);
    }

    public function test_it_purges_dashboard_projection_rates(): void
    {
        ...
        $this->dashboardRatesProjectionRepository->expects($this->once())->method('purgeRates')->with($this->callback(function ($purgeDates) use ($expectedPurgeDates) {
        ...
        $this->sut->purgeAllFrom($purgeDate);
    }
