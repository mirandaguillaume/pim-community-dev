...
final class ChannelLocaleCollection implements \Iterator
{
    ...
    public function __construct(array $localesByChannel)
    {
        foreach ($localesByChannel as $channel => $locales) {
            ...
            $this->localeCollections[$channel] = new LocaleCollection(array_map(fn ($locale): \Akeneo\Pim\Automation\DataQualityInsights\Domain\ValueObject\LocaleCode => new LocaleCode($locale), $locales));
        }
