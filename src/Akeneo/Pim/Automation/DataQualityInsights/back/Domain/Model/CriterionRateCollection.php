...
final class CriterionRateCollection implements \IteratorAggregate
{
    ...
    {
        return array_map(fn ($ratesPerLocale): array => array_map(fn ($rate) => $rate->toInt(), $ratesPerLocale), $this->rates);
    }

    public function toArrayString(): array
    {
        return array_map(fn ($ratesPerLocale): array => array_map(fn ($rate): string => strval($rate), $ratesPerLocale), $this->rates);
    }
