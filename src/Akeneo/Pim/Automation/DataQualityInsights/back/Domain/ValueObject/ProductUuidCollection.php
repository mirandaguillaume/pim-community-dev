...
final class ProductUuidCollection implements ProductEntityIdCollection
{
    ...
    public static function fromStrings(array $productUuids): self
    {
        ...
        return new self(array_map(fn (string $productUuid): \Akeneo\Pim\Automation\DataQualityInsights\Domain\ValueObject\ProductUuid => ProductUuid::fromString($productUuid), $productUuids));
    }
    ...
    {
        return array_map(fn (ProductEntityIdInterface $productUuid): string => (string) $productUuid, $this->productUuids);
    }

    public function toArrayBytes(): array
    {
        return array_map(fn (ProductEntityIdInterface $productUuid): string => $productUuid->toBytes(), $this->productUuids);
    }
}
