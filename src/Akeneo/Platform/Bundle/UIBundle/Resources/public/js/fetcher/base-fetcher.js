...
export default Backbone.Model.extend({
  ...
  fetchAll: function (searchOptions) {
    if (!this.entityListPromise) {
      ...
      this.entityListPromise = $.getJSON(Routing.generate(this.options.urls.list, searchOptions))
        .then(_.identity)
        .promise();
    }
  ...
  fetch: function (identifier, options) {
    ...
    if (!(identifier in this.entityPromises) || false === options.cached) {
      ...
      if (this.options.urls.get && !options.force_list_method) {
        $.getJSON(Routing.generate(this.options.urls.get, _.extend({identifier: identifier}, options)))
          .then(_.identity)
