...
describe('createCategory', () => {
  ...
  it('returns validation errors on failure', async () => {
    ...
    jest.spyOn(global, 'fetch').mockResolvedValueOnce(new Response(JSON.stringify(errors), {status: 422}));
  ...
  it('includes labels when both locale and label are provided', async () => {
    const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValueOnce(new Response('{}', {status: 200}));
