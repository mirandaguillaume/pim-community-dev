...
describe('useCountProductsBeforeDeleteCategory (legacy)', () => {
  ...
  it('calls the callback immediately when numberOfProducts is already known', async () => {
    ...
    await waitFor(() => expect(callback).toHaveBeenCalledWith(5));
  ...
  it('calls the callback with the product count once numberOfProducts loads', async () => {
    ...
    await waitFor(() => expect(callback).toHaveBeenCalledWith(12));
  });
