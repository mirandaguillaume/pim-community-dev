import {convertToCategoryTree, buildTreeNodeFromCategoryTree, CategoryTreeModel} from './Category';

describe('convertToCategoryTree', () => {
  it('parses id by stripping the "node_" prefix', () => {
    const tree = {attr: {id: 'node_42', 'data-code': 'shoes'}, data: 'Shoes', state: 'closed' as const};
    expect(convertToCategoryTree(tree).id).toBe(42);
  });

  it('maps code and label from attr and data fields', () => {
    const tree = {attr: {id: 'node_1', 'data-code': 'master'}, data: 'Master catalog', state: 'closed jstree-root' as const};
  ...
  it('sets isRoot=true when state contains "root"', () => {
    const tree = {attr: {id: 'node_1', 'data-code': 'root'}, data: 'Root', state: 'closed jstree-root' as const};
    expect(convertToCategoryTree(tree).isRoot).toBe(true);
  });

  it('sets isRoot=false when state does not contain "root"', () => {
    const tree = {attr: {id: 'node_1', 'data-code': 'child'}, data: 'Child', state: 'closed' as const};
    expect(convertToCategoryTree(tree).isRoot).toBe(false);
  });

  it('sets isLeaf=true when state is "leaf"', () => {
    const tree = {attr: {id: 'node_5', 'data-code': 'leaf-cat'}, data: 'Leaf', state: 'leaf' as const};
  ...
  it('recursively converts children', () => {
    const tree = {
      ...
      children: [{attr: {id: 'node_2', 'data-code': 'child'}, data: 'Child', state: 'leaf' as const}],
    };
    const result = convertToCategoryTree(tree);
  ...
  it('returns empty children array when children is undefined', () => {
    const tree = {attr: {id: 'node_3', 'data-code': 'alone'}, data: 'Alone', state: 'leaf' as const};
    expect(convertToCategoryTree(tree).children).toStrictEqual([]);
  });
});

describe('buildTreeNodeFromCategoryTree', () => {
  const leafCategory: CategoryTreeModel = {id: 10, code: 'shoes', label: 'Shoes', isRoot: false, isLeaf: true, children: []};
  ...
  it('maps childrenIds from children array', () => {
    const parent: CategoryTreeModel = {
      ...leafCategory,
      isLeaf: false,
      children: [{...leafCategory, id: 11}, {...leafCategory, id: 12}],
    };
  ...
  it('sets childrenStatus="loaded" when children are present', () => {
    const parent: CategoryTreeModel = {...leafCategory, isLeaf: false, children: [{...leafCategory, id: 11}]};
