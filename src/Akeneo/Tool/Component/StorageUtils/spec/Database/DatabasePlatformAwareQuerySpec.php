...
final class DatabasePlatformAwareQuerySpec extends ObjectBehavior
{
    ...
    public function it_generates_mysql_json_array_agg(Connection $connection): void
    {
        ...
        $this->buildJsonArrayAgg('locale.code')->shouldReturn('JSON_ARRAYAGG(locale.code)');
    }
    ...
    public function it_generates_postgresql_json_array_agg(Connection $connection): void
    {
        ...
        $this->buildJsonArrayAgg('locale.code')->shouldReturn('jsonb_agg(locale.code)');
    }
    ...
    {
        $connection->getDatabasePlatform()->willReturn(new MySQLPlatform());
        $this->buildJsonObjectAgg('l.id', 'l.code')->shouldReturn('JSON_OBJECTAGG(l.id, l.code)');
    }

    public function it_generates_postgresql_json_object_agg(Connection $connection): void
    {
        $connection->getDatabasePlatform()->willReturn(new PostgreSQLPlatform());
        $this->buildJsonObjectAgg('l.id', 'l.code')->shouldReturn('jsonb_object_agg(l.id, l.code)');
    }
    ...
    {
        $connection->getDatabasePlatform()->willReturn(new MySQLPlatform());
        $this->buildJsonRemoveKey('doc', 'NO_LOCALE')->shouldReturn("JSON_REMOVE(doc, '$.NO_LOCALE')");
    }

    public function it_generates_postgresql_json_remove_key(Connection $connection): void
    {
        $connection->getDatabasePlatform()->willReturn(new PostgreSQLPlatform());
        $this->buildJsonRemoveKey('doc', 'NO_LOCALE')->shouldReturn("(doc - 'NO_LOCALE')");
    }
    ...
    {
        $connection->getDatabasePlatform()->willReturn(new MySQLPlatform());
        $this->buildRegexpMatch('raw_parameters', ':regex')->shouldReturn('raw_parameters REGEXP :regex');
    }

    public function it_generates_postgresql_regexp_match(Connection $connection): void
    {
        $connection->getDatabasePlatform()->willReturn(new PostgreSQLPlatform());
        $this->buildRegexpMatch('raw_parameters', ':regex')->shouldReturn('raw_parameters ~ :regex');
    }
    ...
    {
        $connection->getDatabasePlatform()->willReturn(new MySQLPlatform());
        $this->buildGroupConcat('currency.code', "'-'")->shouldReturn("GROUP_CONCAT(currency.code SEPARATOR '-')");
    }

    public function it_generates_mysql_group_concat_with_order(Connection $connection): void
    {
        $connection->getDatabasePlatform()->willReturn(new MySQLPlatform());
        $this->buildGroupConcat('currency.code', "'-'", 'currency.code')
    ...
    {
        $connection->getDatabasePlatform()->willReturn(new PostgreSQLPlatform());
        $this->buildGroupConcat('currency.code', "'-'")->shouldReturn("STRING_AGG(currency.code, '-')");
    }
