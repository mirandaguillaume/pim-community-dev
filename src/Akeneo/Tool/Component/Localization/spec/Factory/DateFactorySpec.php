...
class DateFactorySpec extends ObjectBehavior
{
    ...
    public function it_returns_intl_formatter()
    {
        $this->create([])->shouldReturnAnInstanceOf(\IntlDateFormatter::class);
    }

    public function it_creates_a_date_with_intl_format()
    {
        $options = ['locale' => 'fr_FR'];
        $this->create($options)->getPattern()->shouldReturn('dd/MM/yyyy');
    }

    public function it_creates_a_date_with_defined_format()
    {
        $options = ['locale' => 'fr_FR', 'date_format' => 'd/M/yy'];
        $this->create($options)->getPattern()->shouldReturn('d/M/yy');
    }

    public function it_replaces_2_digit_years_by_4_digit_when_the_format_is_not_specified(\IntlDateFormatter $formatter)
    {
