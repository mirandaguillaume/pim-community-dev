...
class JobRegistrySpec extends ObjectBehavior
{
    ...
    public function it_gets_a_job_activated_through_feature_flag(JobInterface $assetJob)
    {
        $this->get('asset_manager_job')->shouldReturn($assetJob);
    }

    public function it_gets_a_job_even_it_is_disabled_through_feature_flag_to_allow_icecat_job_installation(JobInterface $referenceEntityJob)
    {
        $this->get('reference_entity_job')->shouldReturn($referenceEntityJob);
    }

    public function it_return_if_a_job_is_activated_to_make_it_visible_or_not_in_the_process_tracker_for_example(JobInterface $assetJob)
    {
        $this->isEnabled('asset_manager_job')->shouldReturn(true);
    }

    public function it_return_if_a_job_is_disabled_to_make_it_invisible_or_not_in_the_process_tracker_for_example(JobInterface $assetJob)
    {
        $this->isEnabled('reference_entity_job')->shouldReturn(false);
    }

    public function it_throws_an_exception_when_checking_if_an_non_existent_job_is_activated_or_not(JobInterface $referenceEntityJob)
    {
        $this->shouldThrow(UndefinedJobException::class)->during('isEnabled', ['foo']);
    }

    public function it_gets_a_job_when_no_feature_flag_configured_for_it(JobInterface $productExportJob)
    {
        $this->get('product_export_job')->shouldReturn($productExportJob);
    }

    public function it_throws_an_exception_when_getting_a_non_existing_job(JobInterface $referenceEntityJob)
    {
        $this->shouldThrow(UndefinedJobException::class)->during('get', ['foo']);
    }

    public function it_gets_all_activated_jobs_through_feature_flags(JobInterface $assetJob, JobInterface $productExportJob)
    {
        $this->all()->shouldReturn(['asset_manager_job' => $assetJob, 'product_export_job' => $productExportJob]);
    }

    public function it_gets_all_by_type(JobInterface $assetJob, JobInterface $productExportJob)
    {
        $this->allByType('import')->shouldReturn(['asset_manager_job' => $assetJob]);
    }

    public function it_gets_all_by_type_group_by_connector(JobInterface $assetJob, JobInterface $productExportJob)
    {
        $this->allByTypeGroupByConnector('import')->shouldReturn(['connector_2' => ['asset_manager_job' => $assetJob]]);
    }

    public function it_gets_connectors()
    {
