...
class VersionManagerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->om->method('getRepository')->willReturn($this->versionRepository);
    ...
    public function test_it_uses_version_builder_to_build_versions(): void
    {
        ...
        $this->builder->method('buildVersion')->willReturn(new Version('foo', 1, null, 'julia'));
        $versions = $this->sut->buildVersion($product);
    ...
    public function test_it_builds_pending_versions_and_last_version_when_versioning_an_entity(): void
    {
        ...
        $this->om->method('getRepository')->willReturn($this->versionRepository);
        $this->builder = $this->createMock(VersionBuilder::class);
        $this->builder->method('buildPendingVersion')->willReturnCallback(function ($pending, $prev) {
            return $pending;
        });
        $this->builder->method('buildVersion')->willReturn(new Version('Product', null, Uuid::fromString('114c9108-444d-408a-ab43-195068166d2c'), 'julia'));
        $this->sut = new VersionManager($this->om, $this->builder, $this->versionContext, $this->eventDispatcher);
        $versions = $this->sut->buildVersion($product);
        $this->assertIsArray($versions);
