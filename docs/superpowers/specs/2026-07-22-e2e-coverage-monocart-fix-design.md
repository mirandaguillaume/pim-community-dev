...
1. **monocart `add()` input shape.** ✅ RESOLVED during implementation. `includeRawScriptCoverage` / `it.rawScriptCoverage` are **Puppeteer-only — they do NOT exist in Playwright's `page.coverage` API** (verified against `playwright-core` 1.58.2). The flag is silently dropped and `it.rawScriptCoverage` is always `undefined`, so the `{source, ...rawScriptCoverage}` form from lines 19-21/35-36 above would collapse each dump to a source-only entry with no coverage data — silently preserving the degenerate result. **Shipped fix:** dump Playwright's `stopJSCoverage()` entries **as-is** (`JSON.stringify(entries)`) — they already carry `{url, scriptId, source, functions}`, which is exactly the shape monocart's `add()` consumes for Playwright V8 coverage. Validated locally: a synthetic partial-coverage entry through monocart yields `LF=5 / LH=4` (sub-100%), proving monocart reconstructs the full line denominator from `source`. Lines 19-21 and 35-36 of this doc are therefore superseded on the `includeRawScriptCoverage` point.
