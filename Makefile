...
NODE_USER ?= node
NODE_RUN ?= $(DOCKER_COMPOSE) run -u $(NODE_USER) --rm -e YARN_REGISTRY -e PUPPETEER_SKIP_CHROMIUM_DOWNLOAD=1 -e PUPPETEER_EXECUTABLE_PATH=/usr/bin/google-chrome -e YARN_CACHE_FOLDER=/tmp/.yarn node
