<?php

declare(strict_types=1);

namespace Akeneo\Test\Pim\Unit\Enrichment\Component\Product\Validator;

use Akeneo\Pim\Enrichment\Component\Product\Validator\ChainedAttributeConstraintGuesser;
use Akeneo\Pim\Enrichment\Component\Product\Validator\ConstraintGuesserInterface;
use Akeneo\Pim\Structure\Component\Model\AttributeInterface;
use PHPUnit\Framework\TestCase;

class ChainedAttributeConstraintGuesserTest extends TestCase
{
    private ChainedAttributeConstraintGuesser $sut;

    protected function setUp(): void
    {
        $this->sut = new ChainedAttributeConstraintGuesser();
    }

}
