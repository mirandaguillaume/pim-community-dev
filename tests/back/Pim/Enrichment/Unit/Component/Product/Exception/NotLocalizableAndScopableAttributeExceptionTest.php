...
class NotLocalizableAndScopableAttributeExceptionTest extends TestCase
{
    private NotLocalizableAndScopableAttributeException $sut;

    protected function setUp(): void
    {
        $this->sut = NotLocalizableAndScopableAttributeException::fromAttributeChannelAndLocale('description', 'ecommerce', 'en_US');
    }

    public function test_it_is_a_domain_and_templated_error_exception(): void
    {
        $this->assertInstanceOf(NotLocalizableAndScopableAttributeException::class, $this->sut);
    ...
    {
        $this->sut = NotLocalizableAndScopableAttributeException::fromAttributeChannelAndLocale('description', null, null);
        $this->assertSame('The description attribute requires a value per channel (nothing was detected)' .
                    ' but does not require a value per locale (nothing was detected).', $this->sut->getMessage());
    }
}
