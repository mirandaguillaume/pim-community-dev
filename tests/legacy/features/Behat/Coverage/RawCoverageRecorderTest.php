...
final class RawCoverageRecorderTest extends TestCase
{
    ...
    public function test_decode_all_ignores_a_tail_truncated_inside_the_length_prefix(): void
    {
        ...
        // written. The `$offset + LENGTH_BYTES <= $total` loop condition is what handles it — unpack()
        // on a short string would otherwise emit a warning and yield a garbage length.
        $good = RawCoverageRecorder::encode(['/srv/pim/src/A.php' => [3 => 1]]);
        $headerOnly = substr(RawCoverageRecorder::encode(['/srv/pim/src/B.php' => [9 => 1]]), 0, 3);
    ...
    public function test_decode_all_skips_a_corrupt_record_and_keeps_reading_after_it(): void
    {
        ...
        // rather than `break`: the offset has already advanced past the payload, so the records that
        // FOLLOW the corrupt one are still recoverable.
        $garbage = 'not gzip at all';
        $corrupt = pack('N', strlen($garbage)) . $garbage;

        $blob = RawCoverageRecorder::encode(['/srv/pim/src/A.php' => [3 => 1]])
            . $corrupt
            . RawCoverageRecorder::encode(['/srv/pim/src/C.php' => [7 => 1]]);

        self::assertSame(
            [
                ['/srv/pim/src/A.php' => [3 => 1]],
                ['/srv/pim/src/C.php' => [7 => 1]],
            ],
